...
public static class ActionCommand
{
  ...
  public static Attribute GetAttribute(string command_name)
  {
    ...
    if (command_name == Fix.SPHERE_OF_GLORY || command_name == Fix.COMMAND_SPHERE_OF_GLORY) { return Attribute.MonsterAction; }
    if (command_name == Fix.AURORA_PUNISHMENT || command_name == Fix.COMMAND_AURORA_PUNISHMENT) { return Attribute.MonsterAction; }
    if (command_name == Fix.INNOCENT_CIRCLE || command_name == Fix.COMMAND_INNOCENT_CIRCLE) { return Attribute.MonsterAction; }
    if (command_name == Fix.ATOMIC_THE_INFINITY_NOVA || command_name == Fix.COMMAND_ATOMIC_THE_INFINITY_NOVA) { return Attribute.MonsterAction; }
    if (command_name == Fix.ABSOLUTE_PERFECTION || command_name == Fix.COMMAND_ABSOLUTE_PERFECTION) { return Attribute.MonsterAction; }
    if (command_name == Fix.ASTRAL_GATE || command_name == Fix.COMMAND_ASTRAL_GATE) { return Attribute.MonsterAction; }
    if (command_name == Fix.DOUBLE_STANCE || command_name == Fix.COMMAND_DOUBLE_STANCE) { return Attribute.MonsterAction; }
    if (command_name == Fix.DESTRUCTION_OF_TRUTH || command_name == Fix.COMMAND_DESTRUCTION_OF_TRUTH) { return Attribute.MonsterAction; }
    if (command_name == Fix.CHAOTIC_SCHEMA || command_name == Fix.COMMAND_CHAOTIC_SCHEMA) { return Attribute.MonsterAction; }
    if (command_name == Fix.OATH_OF_SEFINE || command_name == Fix.COMMAND_OATH_OF_SEFINE) { return Attribute.MonsterAction; }
  ...
  public static TargetType IsTarget(string command_name)
  {
    ...
    if (command_name == Fix.AURORA_PUNISHMENT || command_name == Fix.COMMAND_AURORA_PUNISHMENT) { return TargetType.AllField; }
    if (command_name == Fix.INNOCENT_CIRCLE || command_name == Fix.COMMAND_INNOCENT_CIRCLE) { return TargetType.Own; }
    if (command_name == Fix.ATOMIC_THE_INFINITY_NOVA || command_name == Fix.COMMAND_ATOMIC_THE_INFINITY_NOVA) { return TargetType.Enemy; }
    if (command_name == Fix.ABSOLUTE_PERFECTION || command_name == Fix.COMMAND_ABSOLUTE_PERFECTION) { return TargetType.Own; }
    if (command_name == Fix.ASTRAL_GATE || command_name == Fix.COMMAND_ASTRAL_GATE) { return TargetType.AllField; }
    if (command_name == Fix.DOUBLE_STANCE || command_name == Fix.COMMAND_DOUBLE_STANCE) { return TargetType.Own; }
    if (command_name == Fix.DESTRUCTION_OF_TRUTH || command_name == Fix.COMMAND_DESTRUCTION_OF_TRUTH) { return TargetType.InstantTarget; }
    if (command_name == Fix.CHAOTIC_SCHEMA || command_name == Fix.COMMAND_CHAOTIC_SCHEMA) { return TargetType.Own; }
    if (command_name == Fix.OATH_OF_SEFINE || command_name == Fix.COMMAND_OATH_OF_SEFINE) { return TargetType.Own; }
    if (command_name == Fix.SPACETIME_INFLUENCE || command_name == Fix.COMMAND_SPACETIME_INFLUENCE) { return TargetType.AllMember; }
    #endregion
  ...
  public static int Cost(string command_name)
  {
    ...
    if (command_name == Fix.PURE_VITALIRY_WATER) { return 0; }
    #endregion
    ...
    if (command_name == Fix.AURORA_PUNISHMENT || command_name == Fix.COMMAND_AURORA_PUNISHMENT) { return 0; }
    if (command_name == Fix.INNOCENT_CIRCLE || command_name == Fix.COMMAND_INNOCENT_CIRCLE) { return 0; }
    if (command_name == Fix.ATOMIC_THE_INFINITY_NOVA || command_name == Fix.COMMAND_ATOMIC_THE_INFINITY_NOVA) { return 0; }
    if (command_name == Fix.ABSOLUTE_PERFECTION || command_name == Fix.COMMAND_ABSOLUTE_PERFECTION) { return 0; }
    if (command_name == Fix.ASTRAL_GATE || command_name == Fix.COMMAND_ASTRAL_GATE) { return 0; }
    if (command_name == Fix.DOUBLE_STANCE || command_name == Fix.COMMAND_DOUBLE_STANCE) { return 0; }
    if (command_name == Fix.DESTRUCTION_OF_TRUTH || command_name == Fix.COMMAND_DESTRUCTION_OF_TRUTH) { return 0; }
    if (command_name == Fix.CHAOTIC_SCHEMA || command_name == Fix.COMMAND_CHAOTIC_SCHEMA) { return 0; }
    if (command_name == Fix.OATH_OF_SEFINE || command_name == Fix.COMMAND_OATH_OF_SEFINE) { return 0; }
  ...
  public static Fix.BuffType GetBuffType(string command_name)
  {
    ...
    if (command_name == Fix.PURE_VITALIRY_WATER) { return Fix.BuffType.None; }
    #endregion
    ...
    if (command_name == Fix.AURORA_PUNISHMENT || command_name == Fix.COMMAND_AURORA_PUNISHMENT) { return Fix.BuffType.Negative; }
    if (command_name == Fix.INNOCENT_CIRCLE || command_name == Fix.COMMAND_INNOCENT_CIRCLE) { return Fix.BuffType.Positive; }
    if (command_name == Fix.ATOMIC_THE_INFINITY_NOVA || command_name == Fix.COMMAND_ATOMIC_THE_INFINITY_NOVA) { return Fix.BuffType.None; }
    if (command_name == Fix.ABSOLUTE_PERFECTION || command_name == Fix.COMMAND_ABSOLUTE_PERFECTION) { return Fix.BuffType.Positive; }
    if (command_name == Fix.ASTRAL_GATE || command_name == Fix.COMMAND_ASTRAL_GATE) { return Fix.BuffType.Negative; }
    if (command_name == Fix.DOUBLE_STANCE || command_name == Fix.COMMAND_DOUBLE_STANCE) { return Fix.BuffType.Positive; }
    if (command_name == Fix.DESTRUCTION_OF_TRUTH || command_name == Fix.COMMAND_DESTRUCTION_OF_TRUTH) { return Fix.BuffType.None; }
  ...
  public static bool IsDamage(string command_name)
  {
    ...
    if (command_name == Fix.PURE_VITALIRY_WATER) { return false; }
    #endregion
    ...
    if (command_name == Fix.AURORA_PUNISHMENT || command_name == Fix.COMMAND_AURORA_PUNISHMENT) { return false; }
    if (command_name == Fix.INNOCENT_CIRCLE || command_name == Fix.COMMAND_INNOCENT_CIRCLE) { return false; }
    if (command_name == Fix.ATOMIC_THE_INFINITY_NOVA || command_name == Fix.COMMAND_ATOMIC_THE_INFINITY_NOVA) { return true; }
    if (command_name == Fix.ABSOLUTE_PERFECTION || command_name == Fix.COMMAND_ABSOLUTE_PERFECTION) { return false; }
    if (command_name == Fix.ASTRAL_GATE || command_name == Fix.COMMAND_ASTRAL_GATE) { return false; }
    if (command_name == Fix.DOUBLE_STANCE || command_name == Fix.COMMAND_DOUBLE_STANCE) { return false; }
    if (command_name == Fix.DESTRUCTION_OF_TRUTH || command_name == Fix.COMMAND_DESTRUCTION_OF_TRUTH) { return false; }
