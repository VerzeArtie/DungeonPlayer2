...
public class DungeonField : MotherBase
{
    ...
    public override void Start()
    {
        ...
        int counter = 0;
        if (One.TF.AvailableAnshet) { txtMapSelect[counter].text = Fix.TOWN_ANSHET; } counter++;
        if (One.TF.AvailableGoratrum) { txtMapSelect[counter].text = Fix.DUNGEON_GORATRUM_CAVE; } counter++;
        if (One.TF.AvailableQvelta) { txtMapSelect[counter].text = Fix.TOWN_QVELTA_TOWN; } counter++;
        if (One.TF.AvailableArtharium) { txtMapSelect[counter].text = Fix.DUNGEON_ARTHARIUM_FACTORY; } counter++;
        if (One.TF.AvailableCotuhsye) { txtMapSelect[counter].text = Fix.TOWN_COTUHSYE; } counter++;
        if (One.TF.AvailableZhalman) { txtMapSelect[counter].text = Fix.TOWN_ZHALMAN; } counter++;
        if (One.TF.AvailableOhran) { txtMapSelect[counter].text = Fix.TOWER_OHRAN; } counter++;
        if (One.TF.AvailableVelgus) { txtMapSelect[counter].text = Fix.DUNGEON_VELGUS_SEA_TEMPLE; } counter++;
        if (One.TF.AvailableArcaneDine) { txtMapSelect[counter].text = Fix.TOWN_ARCANEDINE; } counter++;
        if (One.TF.AvailableParmetysia) { txtMapSelect[counter].text = Fix.TOWN_PARMETYSIA; } counter++;
        if (One.TF.AvailableSaritan) { txtMapSelect[counter].text = Fix.DUNGEON_RUINS_OF_SARITAN; } counter++;
        if (One.TF.AvailableDhal) { txtMapSelect[counter].text = Fix.DUNGEON_GATE_OF_DHAL; } counter++;
        if (One.TF.AvailableDale) { txtMapSelect[counter].text = Fix.TOWN_DALE; } counter++;
        if (One.TF.AvailableDiskel) { txtMapSelect[counter].text = Fix.DUNGEON_DISKEL_BATTLE_FIELD; } counter++;
        if (One.TF.AvailableGanro) { txtMapSelect[counter].text = Fix.DUNGEON_GANRO_FORTRESS; } counter++;
        if (One.TF.AvailableEdelgarzen) { txtMapSelect[counter].text = Fix.TOWN_EDELGARZEN_CASTLE; } counter++;
        if (One.TF.AvailableLoslon) { txtMapSelect[counter].text = Fix.DUNGEON_LOSLON_CAVE; } counter++;
        if (One.TF.AvailableZelman) { txtMapSelect[counter].text = Fix.TOWN_ZELMAN; } counter++;
        if (One.TF.AvailableLataHouse) { txtMapSelect[counter].text = Fix.TOWN_LATA_HOSE; } counter++;
        if (One.TF.AvailableSithGraveyard) { txtMapSelect[counter].text = Fix.DUNGEON_SITH_GRAVEYARD; } counter++;
        if (One.TF.AvailableFran) { txtMapSelect[counter].text = Fix.TOWER_FRAN; } counter++;
        if (One.TF.AvailableSnowtreeLata) { txtMapSelect[counter].text = Fix.DUNGEON_SNOWTREE_LATA; }  counter++;
        if (One.TF.AvailableWosm) { txtMapSelect[counter].text = Fix.TOWN_WOSM; } counter++;
        if (One.TF.AvailableFazilCastle) { txtMapSelect[counter].text = Fix.TOWN_FAZIL_CASTLE; } counter++;
        ...
        // プレイヤーリスト
        int playerCounter = 0;
        if (One.TF.AvailableEinWolence) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableLanaAmiria) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableEoneFulnea) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableMagiZelkis) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableSelmoiRo) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableKartinMai) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableJedaArus) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableSinikiaVeilhanz) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableAdelBrigandy) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableLeneColtos) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailablePermaWaramy) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableKiltJorju) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableBillyRaki) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableAnnaHamilton) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableCalmansOhn) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableSunYu) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableShuvaltzFlore) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableRvelZelkis) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableVanHehgustel) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableOhryuGenma) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableLadaMystorus) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
        if (One.TF.AvailableSinOscurete) { PlayerList.Add(One.Characters[playerCounter]); } playerCounter++;
