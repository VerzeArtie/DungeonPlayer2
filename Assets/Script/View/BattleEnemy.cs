...
//      SetupActionCommandButton(GlobalActionButtonList[ii], str_list0[ii]);
  ...
  private bool ExecPatermaDismagicDrink(Character player, Character target)
  {
    if (One.TF.FindBackPackItem(Fix.PATERMA_DISMAGIC_DRINK) == false)
    {
      Debug.Log(Fix.PATERMA_DISMAGIC_DRINK + " was not found... then miss.");
      StartAnimation(player.objGroup.gameObject, Fix.BATTLE_NO_POTION, Fix.COLOR_NORMAL);
      return false;
    }

    Item current = new Item(Fix.PATERMA_DISMAGIC_DRINK);
    One.TF.DeleteBackpack(current, 1);
  ...
  private double MagicDamageLogic(Character player, Character target, double magnify, Fix.DamageSource attr, Fix.IgnoreType ignore_target_defense, Fix.CriticalType critical, ref bool result_critical)
  {
    ...
    double debug3 = defenseValue;
    damageValue -= defenseValue;
    double debug4 = damageValue;

    Debug.Log("Magic-DamageValue: " + debug0.ToString("F2") + " -> " + debug1.ToString("F2") + " -> " + debug2.ToString("F2") + " - " + debug3.ToString("F2") + " = " + debug4.ToString("F2"));
